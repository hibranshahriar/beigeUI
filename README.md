...
The goal was to make a UI that's basic, lightweight, and easy enough to use for someone who is just getting into running LLMs on their own machine using ollama.
...
- Sidebar for info such as session token count (input token is an estimate) and memory usage.
...
- Thinking process is tucked into an expandable segment on top of the response (for reasoning models).
